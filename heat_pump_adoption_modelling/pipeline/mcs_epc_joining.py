...
def prepare_epcs():
    ...
    epcs["standardised_address"] = [
        rm_punct(address).lower() for address in epcs["ADDRESS1"]
    ]
...
def join_mcs_epc_data(dhps=None, epcs=None, save=True):
    ...
    def epc_lookup(i):
        try:
            ...
            return epcs.loc[index][epc_characteristic_fields + ["standardised_address"]]
        except KeyError:
            return np.nan
    ...
    for i in dhps.index:
        dhps.loc[i, epc_characteristic_fields + ["standardised_address"]] = epc_lookup(
            i
        )
