...
def plot_median_costs(df):
    ...
    ax.set_title(
        "Median cost of heat pump installations\n(years with >100 installations only)"
    )
...
def plot_installation_numbers(df):
    ...
    numbers_data = (
        df.groupby(["year", "tech_type"])["cost"].count().unstack().loc[2014:2021]
    )

    fig, ax = plt.subplots()

    numbers_data.plot(kind="bar", ax=ax, zorder=3, color=["orange", "blue"])
    ax.set_xlabel("Year")
